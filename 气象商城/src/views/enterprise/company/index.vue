	...
	<div class="app-container">
		...
		<el-table :data="announceData" border style="width: 100%;margin-bottom: 10px;" height="710" max-height="780"
		 v-loading="tableLoading" element-loading-text="正在拼命加载中。。。。">
			...
			<el-table-column prop="businessNum" label="营业执照号" min-width="9%" align="center"></el-table-column>
			...
			<el-table-column prop="idcardImages" label="营业执照" min-width="30%" align="center">
				<template slot-scope="scope">
					<viewer>
...
<script>
	...
	export default {
		...
		methods: {
			...
			selectEnterprise() {
				this.announceData = []
				getOwnCompanyInfo().then(res => {
					console.log(res)
					for (var i = 0; i < res.data.length; i++) {
						var image = res.data[i].idcardImages.split(",")
						console.log(image)
						this.$set(res.data[i], 'imagesPositive', image[0])
						this.$set(res.data[i], 'imagesReverse', image[1])
						this.announceData.push(res.data[i])
					}
